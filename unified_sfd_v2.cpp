 ...
 * 5. ESS-adjusted Wilson score confidence intervals
 *
 ...
 */
 ...
 public:
     ...
     Result estimate(size_t target_samples = 0) {
         ...
         // Compute frequencies and ESS-adjusted CI
         computeFrequenciesAndCI(r, chain_type_samples);
 ...
 private:
     ...
     // =================================================================
     // ESS-ADJUSTED WILSON CONFIDENCE INTERVALS
     // =================================================================

     /**
      * Compute effective sample size for one chain's binary indicator series.
      * Uses initial-positive-sequence truncation (Geyer, 1992): accumulate
      * autocorrelation lags until the first negative estimate, then
      * ESS = n / (1 + 2 * cumulative_rho).
      */
     static double computeChainESS(const vector<int>& samples, int type) {
         size_t n = samples.size();
         if (n < 20) return (double)n;

         // Binary indicator: 1 if this sample is the target type, else 0
         vector<double> x(n);
         double mean = 0;
         for (size_t j = 0; j < n; j++) {
             x[j] = (samples[j] == type) ? 1.0 : 0.0;
             mean += x[j];
         }
         mean /= n;
         ...
         var /= n;
         if (var < 1e-12) return (double)n;  // constant → no autocorrelation

         // Autocorrelation, truncated at first negative lag
         double sum_rho = 0;
         size_t max_lag = min(n / 4, (size_t)500);
         for (size_t lag = 1; lag <= max_lag; lag++) {
             double cov = 0;
             for (size_t j = 0; j + lag < n; j++) {
                 cov += (x[j] - mean) * (x[j + lag] - mean);
             }
             cov /= n;
             double rho = cov / var;
             if (rho < 0.0) break;
         ...
         return max(1.0, (double)n / (1.0 + 2.0 * sum_rho));
     }

     /**
      * ESS-adjusted Wilson confidence intervals.
      *
      * For each simplet type, the effective sample size (ESS) is estimated
      * from the autocorrelation of each chain's binary indicator series and
      * summed across chains (valid because chains are independent).
      * The pooled ESS replaces the nominal sample count in the Wilson
      * formula, producing wider, better-calibrated confidence bands.
      */
     void computeFrequenciesAndCI(Result& r,
                                   const vector<vector<int>>& chain_type_samples) {
         double z = 1.96;

         cerr << "\nEffective Sample Size (ESS) per active type:" << endl;

         for (int i = 0; i < NUM_TYPES; i++) {
             r.freq[i] = r.samples > 0 ? (double)r.counts[i] / r.samples : 0;
             double p = r.freq[i];

             // Sum ESS across all chains for type (i+1)
             double ess = 0;
             for (const auto& chain : chain_type_samples) {
                 if (!chain.empty())
                     ess += computeChainESS(chain, i + 1);  // 1-indexed
             }
             ess = max(1.0, ess);

             // Wilson CI with n_eff
             double n_eff = ess;
             double denom = 1.0 + z * z / n_eff;
             double center = (p + z * z / (2.0 * n_eff)) / denom;
             double margin = z * sqrt(p * (1.0 - p) / n_eff
                                      + z * z / (4.0 * n_eff * n_eff)) / denom;
             r.ci_lo[i] = max(0.0, center - margin);
             r.ci_hi[i] = min(1.0, center + margin);

             if (p > 0.001) {
                 cerr << "  Type " << setw(2) << (i + 1)
                      << ": ESS=" << setw(7) << fixed << setprecision(0) << ess
                      << " / " << r.samples
                      << "  (ratio=" << setprecision(3) << ess / r.samples << ")" << endl;
             }
         }
     }
 ...
 bool loadDBLP(SimplicialComplex& K, const string& nverts_file, const string& simplices_file,
     ...
     vector<vector<int>> all_simplices_raw;
     vector<vector<int>> higher_simplices;
 ...
 int main(int argc, char* argv[]) {
     ...
     cerr << "SFD Estimates with 95% CI (ESS-adjusted):" << endl;
     cerr << "-----------------------------------------" << endl;
     for (int i = 0; i < NUM_TYPES; i++) {
         if (r.counts[i] > 0) {
             cerr << "  Type " << setw(2) << (i+1) << ": "
